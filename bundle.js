...
function findScalePositions(targetPcs, tuningSpec, maxFret = 22) {
  ...
  return allRoutes.filter((r) => {
    ...
    seen.add(fp);
    return true;
  }).sort((a, b) => a.windowStart !== b.windowStart ? a.windowStart - b.windowStart : a.notes.length - b.notes.length);
}
function buildScaleRoutes(targetPcs, map, maxFret, notesPerString) {
  ...
  for (let startFret = 0;startFret <= 12; startFret++) {
    if (targetPcs.indexOf(pitchClass(lowE[startFret])) === -1)
      continue;
    const notes = [];
    let valid = true;
...
function renderScaleDiagram(container, notes, windowStart, windowSize) {
  ...
  for (let f = 1;f <= FRETS; f++) {
    svgLine(svg, mLeft + f * FW, mTop, mLeft + f * FW, mTop + boardH, { stroke: cfg.fretColor, "stroke-width": 1.5 });
  }
  for (let s = 0;s < STRINGS; s++) {
